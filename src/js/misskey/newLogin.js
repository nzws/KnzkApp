knzkMk.openLoginConfirm = function(domain) {
  if (domain) {
    if (domain.match(/@/i)) {
...
knzkMk.openLogin = function(domain) {
  var os_name,
...
knzkMk.loginCallback = function() {
  ...
  Fetch('https://' + localStorage.getItem('knzkapp_tmp_domain') + '/api/auth/session/userkey', {
    ...
    .then(function(json) {
      setTimeout(function() {
        if (json.accessToken) {
          now_userconf['token'] = sha256(
            json.accessToken + localStorage.getItem('knzkapp_tmp_scr')
          );
