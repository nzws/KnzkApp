function toot_card(toot, mode, note, toot_light, page) {
  ...
  if (toot['reblog']) {
    ...
    alert_text =
      ...
      t_text(escapeHTML(toot['account']['display_name']), toot['account']['emojis'], toot['account']['acct']) +
      '</b>' +
    ...
    .then(function(json_stat) {
      if (json_stat) {
        if (near) {
          ...
          Fetch('https://' + toot_origin_domain + '/api/v1/timelines/' + near + 'limit=10&max_id=' + origin_id, {
            headers: near_header,
            method: 'GET'
          })
            .then(function(response) {
...
function quote_toot(text, acct) {
  ...
  post_pre(
    text + '\n' + i18next.t('original_toot.quote.prefix') + acct + i18next.t('original_toot.quote.suffix') + '\n',
    1
  )
}
