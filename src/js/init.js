  ...
  if (!localStorage || !Fetch) {
    ...
    if (localStorage.getItem('knzkapp_now_token')) {
      ...
      try {
        ...
        elemId('css_md').href = instance_config[inst]['markdown'] ? 'css/kirishima_markdown.css' : ''
              ...
              try {
                ...
                setTimeout(() => {
                  ...
                  try {
                    ...
                    if (instance_config[inst]['yomigana']) elemId('splitter-profile-name').style.height = '30px'
...
function initevent() {
  ...
  document.addEventListener('postpush', event => {
    ...
    if (event.enterPage.id === 'toot-page') {
      ...
      elemId('post_mode_icon').className = 'ons-icon fa fa-fw fa-' + visibility_icon_name(tmp_post_visibility)
      ...
      if (tmp_post_reply) {
        ...
        elemId('post_reply_acct').innerHTML = tmp_text_pre ? post_reply_acct : i18next.t('toot.reply.me')
