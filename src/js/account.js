...
function show_account(id) {
  ...
  Fetch('https://' + inst + '/api/v1/accounts/' + id, {
    ...
    .then(json => {
      ...
      elemId('userpage-name').innerHTML = t_text(escapeHTML(json.display_name), json.emojis, json.acct)
      ...
      elemId('userpage-icon' + largeId).src = json[getConfig(1, 'no_gif') ? 'avatar_static' : 'avatar']
      document
        .getElementById('userpage-bg' + largeId)
        .setAttribute(
          'style',
          "background-image: url('" + json[getConfig(1, 'no_gif') ? 'header_static' : 'header'] + "');"
        )
      ...
      if (json.username === json.acct) {
        ...
        elemId('userpage-hint').innerHTML = i18next.t('account.joined_at') + ': <b>' + date_text + '</b>'
      } else {
  ...
  Fetch('https://' + inst + '/api/v1/accounts/relationships?id=' + id, {
    ...
    .then(json => {
      ...
      elemId('userpage-follow-button').className =
        'userpage-button ons-icon fa ' + (json[0]['following'] ? 'fa-user-times follow-active' : 'fa-user-plus')

      if (json[0]['muting']) elemId('userpage-follow-button').className = 'invisible'
      elemId('userpage-mute-badge').className = json[0]['muting'] ? 'userpage-follower' : 'invisible'

      if (json[0]['blocking'] === true) elemId('userpage-follow-button').className = 'invisible'
      elemId('userpage-block-badge').className = json[0]['blocking'] === true ? 'userpage-follower' : 'invisible'
  ...
  if (now_userconf['id'] == id) {
    ...
    ons
      ...
      .then(index => {
        ...
        else if (index === 3 && acctdata['rs'][id][0]['following']) account_state_action(id, 'endorsements')
        else if (
