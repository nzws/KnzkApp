function createFilter() {
  var conf = $("[id^='filter_']"), i = 0, config = {}, id = escapeHTML(document.getElementById("userid_filter").value).toLowerCase();
  if (!id) return;
  ...
  for (var key in config) {
    if (key !== "notification") {
      reshtml += "<ons-list-item>" +
        "<div class=\"center\" onclick='editFilter(\"" + key + "\")'><span class=\"list-item__title\">" + key + "</span></div>" +
        " <div class=\"right\" onclick='delFilter(\"" + key + "\")'><span class=\"list-item__title\"><i class=\"list-item__icon list-item--chevron__icon ons-icon fa-trash fa fa-fw\"></i></span></div>\n" +
        "</ons-list-item>\n";
    }
  }
...
function editFilter(id) {
  var config = getConfig_original(5);
  document.querySelector('#navigator').bringPageTop("editFilter.html").then(function () {
    document.getElementById("userid_filter").value = id;
    var conf = $("[id^='filter_']"), i = 0;
    while (conf[i]) {
